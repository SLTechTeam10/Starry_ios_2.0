...
var glowBttn = "SOSGlow"
   ...
   /* func updateReadStatus(id:String , entityName:String){
        ...
        if(entityName == "NotificationEntity"){
            ...
            fetchRequest.predicate = NSPredicate(format: "date = %@", Int(value)!)
        }
...
//       let tapGesture = UITapGestureRecognizer(target: self, action: #selector (self.callSOS))  //Tap function will call when user tap on button
//////        let longGesture = UILongPressGestureRecognizer(target: self, action: #selector(self.callSOS))  //Long function will call when user long press on button.
//       tapGesture.numberOfTapsRequired = 1
//       popUpView?.sosBttn.addGestureRecognizer(tapGesture)
//        popUpView?.sosBttn.addGestureRecognizer(longGesture)
//        popUpView?.sosBttn.showsTouchWhenHighlighted=true
        
        
       // popUpView?.sosBttn.setTitleColor(.red, for: .highlighted)
...
//        }
    ...
    func retrieveNotificationCount() {
        ...
        func retrieveNewsNotificationCount() {
            guard let appDelegate = UIApplication.shared.delegate as? AppDelegate else { return }
            let managedContext = appDelegate.persistentContainer.viewContext
            let fetchRequest = NSFetchRequest<NSManagedObject>(entityName: "NewsletterEntity")
